...
func main() {
	...
	flag.StringVar(&data, "data", "", "Data to POST. If present MHM will use POST requests instead of GET")
	flag.Var(&headers, "header", "Add headers to the request. Could be used multiple times")
	flag.Parse()

	t := os.Getenv("MHMMAXPROCS")
	...
	go func() {
		fmt.Println("-- MHM Attack Started --\n           Go!\n\n")
		...
		for {
			...
			case targetComplete:
				sent++
				fmt.Printf("\r%-6d of max %-6d |\t%7d |\t%6d", cur, maxproc, sent, err)
				fmt.Println("\r-- MHM Attack Finished --       \n\n\r")
				os.Exit(0)
			}
		}
	}()
